...
fun StudyItemIterator.saveState(outState: StudyItemIteratorState) {
    outState.index = index
    outState.curWordId = curWordOrNull?.id ?: ""
    outState.numCorrect = numCorrect
    outState.numWrong = numWrong
}

fun StudyItemIterator.restoreState(state: StudyItemIteratorState) {
    set(
        newIndex = state.index,
        curWordId = state.curWordId,
        newNumCorrect = state.numCorrect,
        newNumWrong = state.numWrong
    )
}
