...
class QAPicker(
    ...
    fun getQA(word: Word, avgLevelOfWords: JLPTLevel, canUseRomaji: Boolean): QuestionAndAnswer? {
        ...
        val phrasesWithWordRemoved = word.phrases.filter { it.canRemoveWordFromPrimaryForm(word) }
        val sentencesWithWordRemoved = word.sentences.filter { it.canRemoveWordFromPrimaryForm(word) }
        val shortPhrases = word.phrases.filter { it.kana.length <= MAX_NUM_CHARS_IN_ANSWER }

        val available = getAvailableKinds(
            word,
            phrasesWithWordRemoved,
            sentencesWithWordRemoved,
            shortPhrases,
            canUseRomaji
        ).map { kind ->
            KindAndWeight(
                kind,
                getWeight(kind, word, phrasesWithWordRemoved, sentencesWithWordRemoved, avgLevelOfWords, canUseRomaji)
            )
        }
    ...
    private fun getAvailableKinds(
        ...
        val hasSeenWord = when {
            isInTestLab -> true // otherwise most kinds would never show up in Google Play Console
    ...
    private fun getWeight(
        ...
        val wkanji = getWeightBasedOnKanji(kind, word, avgLevelOfWords)
