...
class MyWordsMaintainer(private val delegate: Delegate, private val myWordsUnyt: Unyt) {
    ...
    fun onAnswerCorrect(word: Word) {
        ...
        if (myWordsUnyt.numWordsLoaded >= MIN_UNYT_SIZE_FOR_REMOVE && myWordsUnyt.hasWordWithId(word.id)) {
            Log.debug(TAG, "Removing word from My Words unyt: ${word.id}, progress=$progress, rating=$rating")
    ...
    fun onAnswerWrong(word: Word) {
        if (myWordsUnyt.hasWordWithId(word.id)) {
            Log.debug(TAG, "Moving word to the top of My Words unyt: ${word.id}")
            myWordsUnyt.removeAllWithSameId(word)
            myWordsUnyt.add(0, word) // put it back at first index
        } else if (wouldCauseAmbiguity(word)) {
            Log.debug(TAG, "Not adding word to My Words unyt to prevent ambiguities: ${word.id}")
        } else if (myWordsUnyt.numWordsLoaded < IDEAL_UNYT_SIZE) {
            Log.debug(TAG, "Adding word to My Words unyt: ${word.id}")
            myWordsUnyt.add(0, word) // put it back at first index
        } else if (myWordsUnyt.numWordsLoaded > 0 && Random.nextFloat() <= PROBABILITY_FOR_REPLACE) {
            val last = myWordsUnyt.last()
            Log.debug(TAG, "Replacing last in My Words unyt (${last.id}) with ${word.id}")
    ...
    fun onNextWord(newWord: Word) {
        if (myWordsUnyt.hasWordWithId(newWord.id)) {
            Log.debug(TAG, "Word is already in the My Words unyt")
        } else {
            ...
            if (progress >= 1.0f && rating >= MIN_RATING_FOR_REMOVAL_FROM_MY_WORDS) {
                Log.debug(TAG, "Not adding word to My Words unyt, because progress=$progress, rating=$rating")
            } else if (wouldCauseAmbiguity(newWord)) {
                Log.debug(TAG, "Not adding word to My Words unyt to prevent ambiguities")
            } else if (myWordsUnyt.numWordsLoaded < IDEAL_UNYT_SIZE) {
                Log.debug(TAG, "My words unyt has empty slot (loaded ${myWordsUnyt.numWordsLoaded}), adding word")
                myWordsUnyt.add(0, newWord)
            } else {
                val toBeRemoved = findWordToRemoveFromMyWordsUnyt()

                if (toBeRemoved == null) {
                    Log.debug(TAG, "Checked My Words unyt if any words could be removed, but found none")
                } else {
                    ...
                    if (wouldCauseAmbiguity(newWord)) {
                        ...
                        Log.debug(TAG, "Adding word to My Words unyt")
                        myWordsUnyt.add(0, newWord)
                    }
                }
            }
    ...
    companion object {
        ...
        private val TAG = Log.Tag("MyWordsMaintainer")
