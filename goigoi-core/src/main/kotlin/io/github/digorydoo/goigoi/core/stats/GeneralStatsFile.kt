...
class GeneralStatsFile(dir: File): Exportable {
    ...
    fun notifyAppLaunch() {
        ...
        Log.debug(TAG, "App install id: " + file.getString(APP_INSTALL_ID_ID))
        Log.debug(TAG, "App install date: " + file.getString(APP_INSTALL_DATE_ID))
        Log.debug(TAG, "App launch count: $launchCount")
    }
    ...
    fun notifyMainActivityResume() {
        ...
        while (toGo > 0 && m < keepUntil) {
            Log.debug(TAG, "Deleting ${m.formatAsZoneAgnosticDateTime()} (was: ${getUserStudyCountOfDay(m)})")
