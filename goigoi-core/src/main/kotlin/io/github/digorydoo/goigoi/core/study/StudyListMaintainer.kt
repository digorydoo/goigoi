...
import io.github.digorydoo.goigoi.core.study.MyWordsMaintainer.Companion.MIN_RATING_FOR_REMOVAL_FROM_MY_WORDS
...
class StudyListMaintainer(
    ...
    fun onAnswerCorrect(word: Word): Boolean {
        ...
        if (progress < 1.0f || rating < MIN_RATING_FOR_REMOVAL_FROM_MY_WORDS) {
            ...
            Log.debug(TAG, "Not removing word, because list size is ${list.size}")
            removed = false
        } else if (word.filename == filenameAtHead) {
            Log.debug(TAG, "Not removing word, because it is still the super progressive head")
            removed = false
        } else {
            Log.debug(TAG, "Removing word from study list: ${word.id}, progress=$progress, rating=$rating")
            list.removeAll { it.word.id == word.id }
            removed = true
        }

        val (add, reasonForAdd) = when {
        ...
        if (add) {
            Log.debug(TAG, "We could add more words, because $reasonForAdd")

            val roomForNew = countRoomForNewWords()

            if (roomForNew > 0) {
                Log.debug(TAG, "Adding one from head as there is room for $roomForNew new word(s)")
                pool.addOneToListFromHead(list, IDEAL_LIST_SIZE)
                roundsSinceAddMoreWords = 0
            } else {
                Log.debug(TAG, "There are too many new or poorly rated words already")

                if (list.size < MIN_INITIAL_LIST_SIZE) {
                    Log.debug(TAG, "List size is ${list.size} <= $MIN_INITIAL_LIST_SIZE: Adding one from the far past")
                    pool.addOneToListFromFarPast(list, IDEAL_LIST_SIZE)
                    roundsSinceAddMoreWords = 0
                } else if (Random.nextFloat() <= PROBABILITY_FOR_ADD_WORD_FROM_FAR_PAST) {
                    Log.debug(TAG, "rnd <= $PROBABILITY_FOR_ADD_WORD_FROM_FAR_PAST: Adding one from the far past")
                    pool.addOneToListFromFarPast(list, IDEAL_LIST_SIZE)
                    roundsSinceAddMoreWords = 0
                } else {
                    Log.debug(TAG, "rnd > $PROBABILITY_FOR_ADD_WORD_FROM_FAR_PAST: Not adding any more words")
                }
            }

            ensureSuperProgressiveHeadIsInListAndEarly() // in order not to keep pushing it back
        } else {
            Log.debug(TAG, "Not adding more words, because $reasonForAdd")
        }
    ...
    fun onAnswerWrong() {
        ...
        if (superProgressive && list.size > MIN_LIST_SIZE_FOR_REMOVE) {
            // If there are any words in the list that are completely new, drop the first one.

            val filenameAtHead = pool.filenameAtHead

            val drop = list.firstOrNull {
                it.word.filename != filenameAtHead && stats.getWordStudyProgress(it.word) == 0.0f
            }

            if (drop != null) {
                Log.debug(TAG, "Dropping new word ${drop.word.id} from study list, because answer was wrong")
                list.remove(drop)
            }
        }
    }
    ...
    fun pushBack(item: StudyItem, answer: Answer) {
        ...
        if (step <= 0) {
            Log.warn(TAG, "Unexpected step value: $step")
        } else {
            Log.debug(TAG, "Putting item back by $step")
            list.remove(item)
    ...
    private fun limitStepForWordAtHead(step: Int, word: Word) =
        if (superProgressive && pool.isHeadAt(word)) {
            Log.debug(TAG, "Limiting the push step of ${word.id} since it's the super progressive head")
    ...
    private fun ensureSuperProgressiveHeadIsInListAndEarly() {
        ...
        if (filename == null) {
            Log.warn(TAG, "There is no word at super prog head")
            return
        }
        ...
        if (wordAtHead == null) {
            Log.warn(TAG, "Failed to load word ahead: $filename")
            return
        }
        ...
        if (itemAtHead != null) {
            ...
            if (idxOfItemAtHead <= POS_FOR_NEW_WORD) {
                Log.debug(TAG, "Leaving the super prog head (${wordAtHead.id}) at pos $idxOfItemAtHead")
            } else {
                Log.debug(TAG, "Moving the super prog head (${wordAtHead.id}) to pos $POS_FOR_NEW_WORD")
                list.removeAll { it == itemAtHead }
                list.add(min(POS_FOR_NEW_WORD, list.size), itemAtHead)
            }
        } else {
            Log.debug(TAG, "Adding the super prog head (${wordAtHead.id}) to the study list")
            list.add(min(POS_FOR_NEW_WORD, list.size), StudyItem.create(wordAtHead, stats))
        }

        if (list.size > IDEAL_LIST_SIZE) {
            val last = list.last()
            Log.debug(TAG, "Study list has become too large, dropping the last item: ${last.word.id}")
            list.remove(last)
        }
    }
    ...
    fun initForSuperProgressiveMode(myWordsUnyt: Unyt) {
        ...
        if (list.size < MIN_INITIAL_LIST_SIZE) {
            ...
            if (numNewToAdd > 0) {
                Log.debug(TAG, "List size is only ${list.size}, but there's room for $numNewToAdd new words.")
            ...
            if (list.size < MIN_INITIAL_LIST_SIZE) {
                Log.debug(TAG, "List size is only ${list.size}. Trying to add words from the recent past.")
                pool.fillListFromRecentPast(list, MIN_INITIAL_LIST_SIZE)

                if (list.size < MIN_INITIAL_LIST_SIZE) {
                    Log.debug(
                        TAG,
                        "List size is only ${list.size}. Trying to add more words from head with max rating."
                    )
                    pool.fillListFromHead(
                        list,
                    ...
                    if (list.size < MIN_INITIAL_LIST_SIZE) {
                        Log.debug(
                            TAG,
                            "List size is only ${list.size}. Trying to add words from head without max rating."
                        )
                        pool.fillListFromHead(
                            list,
                            MIN_INITIAL_LIST_SIZE,
                        ...
                        if (list.size < MIN_INITIAL_LIST_SIZE) {
                            Log.warn(TAG, "List is still too short: ${list.size} < $MIN_INITIAL_LIST_SIZE")
                        }
                    }
                }
            }
    ...
    companion object {
        private val TAG = Log.Tag("StudyListMaintainer")
