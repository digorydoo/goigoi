...
class StringsFile(val path: String) {
    ...
    fun open() {
        if (access != null) {
            Log.debug(TAG, "File already open")
            return
        }
        ...
        try {
            ...
            if (!f.exists()) {
                Log.debug(TAG, "Creating new file: $path")
                f.createNewFile()
            }
    ...
    fun readln(): String? {
        ...
        try {
            if (access.filePointer >= access.length()) {
                Log.error(TAG, "readln: At end of file!")
                return null
            }
            ...
            if (end != END_MARK) {
                Log.error(TAG, "readln: Missing end mark!")
                return null
            }

            return String(buf, utf8)
        } catch (e: Exception) {
            Log.debug(TAG, "readln: Exception: ${e::class.java.name} ${e.message}")
            return null
        }
    }
    ...
    fun writeln(bytes: ByteArray) {
        ...
        try {
            ...
            Log.debug(TAG, "writeln: Exception: ${e.message}")
            return
        }
    }
    ...
    fun overwriteln() {
        ...
        try {
            ...
            if (end != END_MARK) {
                Log.error(TAG, "overwriteln: Missing end mark!")
                return
            }
            ...
            if (end != END_MARK) {
                Log.error(TAG, "overwriteln: End mark was overwritten!")
                return
            }
        } catch (e: Exception) {
            Log.debug(TAG, "writeln: Exception: ${e.message}")
    ...
    companion object {
        private val TAG = Log.Tag("StringsFile")
