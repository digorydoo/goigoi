...
class FixedKeysProvider(
    ...
    private fun getWordWithIncorrectKanjis(): List<String> {
        ...
        Log.debug(TAG, "allPermutations: ${allPermutations.joinToString(", ")}")
    ...
    private fun getWords(): List<String> {
        ...
        for (i in 0 .. 1) {
            ...
            qa.word.hintsWithSystemLang
                ...
                .let { hintOfQ ->
                    ...
                    if (numAdded > 0) {
                        Log.debug(TAG, "Added $numAdded words with hint $hintOfQ, sameLen=$requireSameLength")
                    }
                }
            ...
            qa.word.cats
                ...
                ?.let { catsOfQ ->
                    ...
                    if (numAdded > 0) {
                        Log.debug(TAG, "Added $numAdded words with categories $catsOfQ, sameLen=$requireSameLength")
                    }
                }
        ...
        if (!enough()) {
            ...
            result.clear()
            Log.debug(TAG, "Got too few specific words, starting over again")
            ...
            for (i in 0 .. 1) {
                ...
                if (numAdded > 0) {
                    Log.debug(TAG, "Added $numAdded arbitrary words, sameLen=$requireSameLength")
                }
    ...
    companion object {
        private val TAG = Log.Tag("FixedKeysProvider")
